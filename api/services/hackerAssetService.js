...
const MANIFEST_DIR = path.join(__dirname, '../data/hacker-assets');
...
/**
 * Resolves a Hacker asset's Drive file ID from the manifest, without touching
 ...
 * @param {string} testId - e.g. "vol-2-test-01".
...
const resolveAsset = (testId, filename) => {
  ...
  if (!fileId) {
    throw createServiceError(404, 'Hacker asset not found.');
  }
...
const CACHE_BUDGET_BYTES = Number(process.env.HACKER_ASSET_CACHE_MB || 150) * 1024 * 1024;
