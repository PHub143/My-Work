...
import YbmReadingContent from './YbmReadingContent.jsx';
import {
  clearAttempt,
  getAnswerKey,
  getAssetUrl,
...
const ExamRunner = ({ test, volumeId }) => {
  ...
  // data/ybm/content/AGENTS.md. Gated per-part (not per-section) because a
  // test can have Reading Parts 5-7 transcribed while Listening stays
  ...
  // "Next" used to dead-end at the booklet's last physical page even when a
  // later part in the same section does have transcribed content — leaving
  // no way to reach it short of clicking directly into the answer sheet.
  // This finds that next content-bearing part, if any, so Next can hand off
  // into the structured view instead of just disabling at the last page.
  const nextContentPart = useMemo(() => {
    if (!readingContent || useStructuredContent) return null;
    const sectionPartsList = PARTS.filter((entry) => entry.section === section);
    const currentIndex = sectionPartsList.findIndex((entry) => entry.part === focusPart);
    for (let i = currentIndex + 1; i < sectionPartsList.length; i += 1) {
      if (readingContent.parts?.[String(sectionPartsList[i].part)]) return sectionPartsList[i];
    }
    return null;
  }, [readingContent, useStructuredContent, section, focusPart]);

  // The reverse handoff: from the *first* transcribed unit of a section,
  // Prev has nowhere to go within the transcribed units even though an
  // earlier image-only part's scanned pages are still right before it.
  // Finds the nearest earlier image-only part, if any, so Prev can hand
  // back into the booklet image instead of just disabling.
  ...
  // Once a later part of this section is fully transcribed, its raw scanned
  // pages are never shown, so there's no reason to page through — or
  // preload — the rest of the booklet's images while still on an
  // image-only part. `imageOnlyPages` records how many leading pages
  ...
  // section's transcribed content, offer a way into the next section rather
  // than just disabling Next.
